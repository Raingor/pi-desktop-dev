...
**Status Legend:** ✅ 已完成 | 🔄 骨架完成 | ⬜ 未开始

| Milestone | Timeline (weeks) | Status | Goals / Deliverables |
|-----------|------------------|--------|----------------------|
| **0. Project Setup** | 1 | **✅ 已完成** | - Repo init, CI/CD pipeline, basic Tauri + React scaffold.<br>• **Add Mac targets:** `rustup target add aarch64-apple-darwin x86_64-apple-darwin`.<br>• Configure CI matrix for Mac builds (Intel + Apple Silicon).<br>• ESLint/Prettier/Clippy setup.<br>• Initial README & CONTRIBUTING. |
| **0.5. Pi Discovery & Onboarding** | 1 | **✅ 完成** | - `PiBridge` module: spawn `pi --mode rpc`, JSONL framing, request/response correlation via `id`, event demux. ✅<br>• Pi binary discovery (bundled → `PI_BINARY` env → PATH). ✅<br>• "Install Pi" onboarding screen if no binary found. ✅<br>• Crash recovery: auto‑restart + `switch_session` to last session. ✅<br>• Smoke test: send `get_state`, render `session` header in UI. ✅<br>• stdout 异步读取线程 ✅ |
| **1. Core Chat UI** | 2‑3 | **✅ 完成** | - Chat window with message list, input box, markdown rendering. ✅<br>• `pi:prompt` / `pi:event` wiring; render `message_update` `text_delta` as streaming tokens. ✅<br>• `pi:abort` Stop button; `pi:new_session` "New Chat". ✅<br>• 消息历史加载 (`get_messages`) ✅ |
| **2. Provider & Model Configuration** | 4 | **✅ 完成** | - Provider Setup panel: call `pi:get_available_models`; render dropdown. ✅<br>• `pi:set_model` / `pi:set_thinking_level`. ✅<br>• API‑key provider entry writes to `~/.pi/agent/auth.json` directly (plain JSON). ✅<br>• Subscription OAuth providers instructions. ✅<br>• llama.cpp provider discovered via `get_available_models`. ⬜ |
| **3. Real‑Time Streaming & Queue** | 5 | **✅ 完成** | - Full event wiring: `agent_start`/`turn_start`/`message_*`/`turn_end`/`agent_end`, `tool_execution_*`, `queue_update`, `compaction_*`, `auto_retry_*`. ✅<br>• `pi:steer` / `pi:follow_up` 模式切换与 mid‑stream UI. ✅<br>• 队列芯片（`queue_update` steering + followUp）、压缩横幅（`compaction_start/end`）。 ✅<br>• Retry banner with countdown + `pi:abort_retry` 取消按钮。 ✅ |
| **4. File Attachments & Preview** | 6 | **✅ 完成** | - Drag‑&‑drop file input. ✅<br>• Image preview, base64 encoding. ✅<br>• code syntax highlighting, PDF thumbnail. ⬜<br>• Images encoded base64 and passed via `prompt`/`steer`/`follow_up` `images` field. ✅ |
| **5. System Tray / Menu Bar & Hotkey** | 7 | **✅ 已完成** | - System tray icon with context menu (New Chat, Settings, Quit). ✅<br>• Global shortcut to show/hide window (configurable). ⬜<br>• **已实现 macOS 菜单栏托盘 + 右键菜单** |
| **6. Session Tree, Fork & Stats** | 8 | **✅ 完成** | - Sidebar lists sessions by scanning `~/.pi/agent/sessions/--<cwd‑slug>--/*.jsonl`. ✅<br>• `pi:switch_session` / `pi:fork`. ✅<br>• `pi:get_session_stats` in sidebar footer. ✅ |
| **7. Notifications** | 9 | **⬜ 未开始** | - Desktop toast for new messages when app is minimized.<br>• Badge count on tray icon. |
| **8. Export / Import & Backup** | 10 | **⬜ 未开始** | - Export: HTML via `pi:export_html`; Markdown/JSON via `pi:get_entries`.<br>• Import: place `.jsonl` into Pi's session dir.<br>• App settings backup. |
| **9. Polishing & Accessibility** | 11 | **⬜ 未开始** | - Keyboard navigation, ARIA labels, focus traps.<br>• High‑contrast theme support.<br>• Performant virtualized list for long chats. |
| **10. Testing & CI/CD** | 12 | **⬜ 未开始** | - Unit & E2E test coverage ≥80%.<br>• Automated builds for Windows/macOS (universal)/Linux.<br>• Signing scripts for macOS universal binary notarization.<br>• CI validates `lipo -info`. |
| **11. Beta Release** | 13 | **⬜ 未开始** | - Distribute to internal testers via GitHub Releases.<br>• Collect feedback, fix critical bugs. |
| **12. Public Release (v1.0)** | 14 | **⬜ 未开始** | - Publish to app stores.<br>• Announcement blog/post. |
| **13. Post‑Launch Monitoring** | Ongoing | **⬜ 未开始** | - Monitor telemetry, crash reports, user feedback.<br>• Iterate based on usage data.<br>• Track crash reports by Mac architecture. |
