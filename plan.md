...
| **0.5. Pi Discovery & Onboarding** | 1 | **🔄 骨架完成** | - `PiBridge` module: spawn `pi --mode rpc`, JSONL framing, request/response correlation via `id`, event demux. ✅<br>• Pi binary discovery (bundled → `PI_BINARY` env → PATH). ✅<br>• "Install Pi" onboarding screen if no binary found. ✅<br>• Crash recovery: auto‑restart + `switch_session` to last session. ⬜<br>• Smoke test: send `get_state`, render `session` header in UI. 🔄<br>• **stdout 异步读取线程未实现** ⬜ |
| **1. Core Chat UI** | 2‑3 | **🔄 骨架完成** | - Chat window with message list, input box, markdown rendering. ✅<br>• `pi:prompt` / `pi:event` wiring; render `message_update` `text_delta` as streaming tokens. 🔄（客户端模拟，未接入真实事件）<br>• `pi:abort` Stop button; `pi:new_session` "New Chat". ✅<br>• **消息历史加载 (`get_messages`) 未实现** ⬜ |
| **2. Provider & Model Configuration** | 4 | **⬜ 未开始** | - Provider Setup panel: call `pi:get_available_models`; render dropdown. ⬜<br>• `pi:set_model` / `pi:cycle_model` / `pi:set_thinking_level` / `pi:cycle_thinking_level`. ⬜<br>• API‑key provider entry writes to `~/.pi/agent/auth.json` directly (plain JSON). ⬜<br>• Subscription OAuth providers instructions. ⬜<br>• llama.cpp provider discovered via `get_available_models`. ⬜ |
| **3. Real‑Time Streaming & Queue** | 5 | **⬜ 未开始** | - Full event wiring: `agent_start`/`turn_start`/`message_*`/`turn_end`/`agent_end`, `tool_execution_*`, `queue_update`, `compaction_*`, `auto_retry_*`.<br>• `pi:steer` / `pi:follow_up` with mid‑stream UI.<br>• `pi:set_steering_mode` / `pi:set_follow_up_mode`.<br>• Retry banner with countdown + `pi:abort_retry`. |
| **4. File Attachments & Preview** | 6 | **⬜ 未开始** | - Drag‑&‑drop file input.<br>• Image preview, code syntax highlighting, PDF thumbnail.<br>• Images encoded base64 and passed via `prompt`/`steer`/`follow_up` `images` field. |
...
| **6. Session Tree, Fork & Stats** | 8 | **⬜ 未开始** | - Sidebar lists sessions by scanning `~/.pi/agent/sessions/--<cwd‑slug>--/*.jsonl`. 🔄（基础扫描已完成）<br>• `pi:switch_session` / `pi:fork` / `pi:clone` / `pi:get_fork_messages`. ⬜<br>• `pi:get_session_stats` in sidebar footer. ⬜ |
